...
const mb = menubar({
  ...
  webPreferences: {
    partition: "persist:ytmusicplayer",
    webgl: true,
  },
});
  ...
  win.webContents.once("dom-ready", () => {
    setInterval(() => {
      ...
      const artistName = win.webContents
        .executeJavaScript(
          `document.querySelector('.ytm-player-controls .artist-name').innerText`
        )
      ...
      const trackName = win.webContents
        .executeJavaScript(
          `document.querySelector('.ytm-player-controls .track-name').innerText`
        )
        .then((result) => result.trim());
      ...
      const playerState = win.webContents
        .executeJavaScript(
          `document.querySelector('.ytm-player-controls .play-pause-btn').getAttribute('title')`
        )
        .then((result) => result.trim())
